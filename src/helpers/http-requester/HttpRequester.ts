...
export interface IHttpRequestExecuteResult {
    response: any,
    error: any,
    success: boolean,
    status?: null | number
...
class HttpRequester {
    ...
    constructor({ library, token = "", refreshToken = "", timeout = 5000, cancelSource = null }: IHttpRequesterConstructor) {
    ...
    public async execute({ method = "GET", url, data }: IHttpRequest) {
        ...
        const source = this.library.CancelToken.source();
        
        this.setCancelSource(source);

        try {
            const res = await this.library.request({
